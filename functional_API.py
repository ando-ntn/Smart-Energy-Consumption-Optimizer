...
#           action from action taken previously ()
            ...
            reward = agent.reward_policy(total_data[i,2])
...
#             "State (T+1)"   
        ...
        reward = agent.reward_policy(self.reward_index[-1], self.q_heater[-1])
...
#             self.state is the old state and state is the new_state
            replay.record(self.state, state, np.int(action), reward, done)
            ...
            experience.append((replay.experiences[-1][0][0], replay.experiences[-1][0][1],replay.experiences[-1][1][0], replay.experiences[-1][1][1]
                                  ,replay.experiences[-1][2], replay.experiences[-1][3]))   
...
#             "to make sure training run on batch or else it won't train"
            if len(replay.experiences) > batch_size : 
                state_batch, next_state_batch, reward_batch, action_batch, done_batch = replay.sample_batch(batch_size)
                loss = agent.train(state_batch = state_batch, next_state_batch = next_state_batch
                           , action_batch = action_batch, reward_batch = reward_batch
                           , done_batch = done_batch, batch_size = batch_size)
#             "save checkpoint every 50 iteration and update target_q_network"
            ...
            if self.counter % 100 == 0 : 
                agent.save_checkpoint()
                agent.save_model(np.sum(functional.reward_step))
                agent.update_target_network()
                "decay rate"
...
#             t_room_final, q_heat = simulation.thermostat_auto(t_room, t_outdoor, auto_bool)
            t_room_final, q_heat = simulation.thermostat_relay(t_room, t_outdoor, t_set_mode, mdot_mode, force_off)
            ...
            self.cost_heater.append(-q_heat*cost)
